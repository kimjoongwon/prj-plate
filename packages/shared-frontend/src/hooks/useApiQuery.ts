		...
		if (pathParams) {
			Object.keys(pathParams).forEach((key) => {
				...
				logger.debug(`🎯 Processing pathParam ${key} -> ${statePath}`, {
					...
					fromUrlParams:
						urlParams?.[statePath] || urlParams?.[key] || "not found",
					finalValue: value,
				});

				if (value === undefined) {
					logger.warning(
						`🔍 PathParam value not found for ${key} -> ${statePath}`,
					);
				}
...
export const useListQuery = (builder: ApiQueryBuilder): ApiQueryResult => {
	...
	try {
		...
		if (!valueField || !labelField) {
			logger.error("📋 Missing list options fields", {
				valueField,
				labelField,
			});
			showToast.error(
				"리스트 쿼리 설정 오류",
				"valueField와 labelField가 필요합니다.",
			);
			return { options: [], isLoading: false };
		}
		...
		const options = data.map((item: any, index: number) => {
			...
			if (value === undefined) {
				logger.warning(
					`📋 Item missing valueField '${valueField}' at index ${index}`,
					item,
				);
			}
	...
	try {
		...
		if (error) {
			...
			logger.success("🗂️ Resource data loaded successfully", {
				...
				hasData: !!data,
				dataKeys:
					typeof data === "object" && data ? Object.keys(data) : "not object",
			});
		} else if (type === "create") {
		...
		return {
			...
			isLoading: false,
			error: error instanceof Error ? error : new Error("Unknown error"),
			type: getTypeFromPath(location.pathname) as
				| "create"
				| "modify"
				| "detail"
				| "add",
