...
describe("NavigatorStore", () => {
  ...
  describe("getRouteByFullPath 메서드", () => {
    ...
    it("1단계 자식 경로를 찾을 수 있어야 함", () => {
      const result = navigatorStore.getRouteByFullPath("/dashboard/analytics", mockRoutes);
    ...
    it("2단계 자식 경로를 찾을 수 있어야 함", () => {
      const result = navigatorStore.getRouteByFullPath("/users/list/detail", mockRoutes);
    ...
    it("존재하지 않는 경로는 undefined를 반환해야 함", () => {
      const result = navigatorStore.getRouteByFullPath("/nonexistent", mockRoutes);
    ...
    it("유사하지만 다른 경로는 찾지 못해야 함", () => {
      const result = navigatorStore.getRouteByFullPath("/dashboard/analytic", mockRoutes);
    ...
    it("동일한 이름의 경로가 있을 때 첫 번째로 일치하는 경로를 반환해야 함", () => {
      ...
      const result = navigatorStore.getRouteByFullPath("/test1", duplicateRoutes);
  ...
  describe("엣지 케이스", () => {
    it("null children이 있는 경로에서 검색이 정상 작동해야 함", () => {
      ...
      const result = navigatorStore.getRouteByFullPath("/parent", routesWithNullChildren);
    ...
    it("빈 children 배열이 있는 경로에서 검색이 정상 작동해야 함", () => {
      ...
      const result = navigatorStore.getRouteByFullPath("/parent", routesWithEmptyChildren);
    ...
    it("특수문자가 포함된 경로를 찾을 수 있어야 함", () => {
      ...
      const result = navigatorStore.getRouteByFullPath("/special-route_with.dots", specialRoutes);
