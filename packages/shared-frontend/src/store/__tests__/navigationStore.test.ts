...
describe("NavigationStore", () => {
  ...
  describe("생성자", () => {
    ...
    it("routeDtos로부터 경로가 초기화되어야 함", () => {
      navigationStore = new NavigationStore(plateStore, navigatorStore, mockRoutes);
  ...
  describe("엣지 케이스", () => {
    ...
    it("깊게 중첩된 경로를 처리해야 함", () => {
      ...
      navigationStore = new NavigationStore(plateStore, navigatorStore, deepRoutes);
      ...
      expect(navigationStore.routes[0].children[0].children[0].active).toBe(true); // Level3
    });
  });
  ...
  describe("initializeCurrentPath", () => {
    ...
    it("pathname이 undefined일 때 빈 문자열을 사용해야 함", () => {
      ...
      vi.spyOn(navigatorStore, "getRouteByFullPath").mockImplementation((path) => {
        expect(path).toBe("");
        return undefined;
      });
      ...
      expect(navigatorStore.getRouteByFullPath).toHaveBeenCalledWith("", expect.any(Array));
    });
  });
});
