...
describe("AuthStore", () => {
  ...
  describe("응답 인터셉터", () => {
    ...
    beforeEach(() => {
      const responseCall = (AXIOS_INSTANCE.interceptors.response.use as any).mock.calls[0];
  ...
  describe("요청 인터셉터", () => {
    ...
    beforeEach(() => {
      const requestCall = (AXIOS_INSTANCE.interceptors.request.use as any).mock.calls[0];
  ...
  describe("logout 메서드", () => {
    it("로그아웃 API 호출 성공 시 스토리지를 클리어하고 로그인 페이지로 이동해야 함", async () => {
      ...
      expect(BrowserUtil.clearLocalStorage).toHaveBeenCalled();
      expect(BrowserUtil.clearSessionStorage).toHaveBeenCalled();
      expect(BrowserUtil.navigateTo).toHaveBeenCalledWith("/admin/auth/login", true);
      expect(authStore.isLoggingOut).toBe(false);
    });
