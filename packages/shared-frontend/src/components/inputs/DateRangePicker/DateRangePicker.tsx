...
import { DateRangePickerProps, MobxProps } from "@shared/types";
...
export const DateRangePicker = observer(<T extends object>(props: DateRangePickerProps<T>) => {
  ...
  const startDateTime = get(state, (path as string)?.split(",")?.[0]) || new Date().toISOString();
  const endDateTime = get(state, (path as string)?.split(",")?.[1]) || new Date().toISOString();
  ...
  useEffect(() => {
    const disposer = reaction(
      ...
      () => {
        set(state, (path as string)?.split(",")?.[0], localState.startDateTime.toAbsoluteString());
        set(state, (path as string)?.split(",")?.[1], localState.endDateTime.toAbsoluteString());
      },
    );
