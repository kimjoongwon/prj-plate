...
describe("CalendarState", () => {
	...
	describe("월 네비게이션", () => {
		it("handlePrevMonth가 이전 달로 이동해야 한다", () => {
			...
			expect(calendarState.displayMonth).toBe(initialMonth === 1 ? 12 : initialMonth - 1);
		});
		...
		it("handleNextMonth가 다음 달로 이동해야 한다", () => {
			...
			expect(calendarState.displayMonth).toBe(initialMonth === 12 ? 1 : initialMonth + 1);
		});
	...
	describe("날짜 생성", () => {
		...
		it("현재 월의 날짜들이 isPressable=true여야 한다", () => {
			const currentMonthDates = calendarState.dates.filter(date => date.isPressable);
		...
		it("이전/다음 월의 날짜들이 isPressable=false여야 한다", () => {
			const otherMonthDates = calendarState.dates.filter(date => !date.isPressable);
	...
	describe("선택된 날짜 처리", () => {
		...
		it("선택되지 않은 날짜는 selected=false여야 한다", () => {
			...
			const unselectedItems = calendarState.dates.filter(date => !date.selected);
