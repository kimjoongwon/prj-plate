...
export const ResourceBuilder = observer((props: ResourceBuilderProps) => {
  ...
  logger.info("🚀 Initializing ResourceBuilder", {
    ...
    props: {
      ...
      elements: elements ? `${elements.length} elements` : "no elements",
    },
  });
  ...
  // 📝 Form input 초기화 - modify/detail 타입일 때 데이터를 form.inputs에 할당
  useEffect(() => {
    if (data && state && type && ["modify", "detail"].includes(type)) {
      try {
        ...
        logger.success("📝 Form input initialized with data", {
          type,
          dataKeys: Object.keys(data || {}),
        });
      } catch (err) {
        logger.error("📝 Failed to initialize form input", err);
      }
    }
  }, [data, state, type]);
    ...
    if (type === "add" && id && state) {
      try {
        ...
        if (!state.form.inputs) {
          state.form.inputs = {};
          logger.debug("📝 Created form input object for parentId");
        }
  ...
  const shouldHaveData =
    !isLoading && type && ["modify", "detail"].includes(type) && id && id !== "new";
