...
import { Prisma, UserAssociation, UseEntity } from "@shared/schema";
...
export class UserAssociationsRepository {
	...
	async create(args: Prisma.UserAssociationCreateArgs): Promise<UserAssociation> {
	...
	async findFirst(args: Prisma.UserAssociationFindFirstArgs): Promise<UserAssociation> {
		this.logger.debug(`UserAssociation 최초 조회 중...`);
		const result = await this.prisma.userAssociation.findFirst(args);
		return plainToInstance(UserAssociation, result);
	}

	async findUnique(args: Prisma.UserAssociationFindUniqueArgs): Promise<UserAssociation> {
	...
	async createManyAndReturn(args: Prisma.UserAssociationCreateManyArgs): Promise<UserAssociation[]> {
		this.logger.debug(`UserAssociation 다중 생성 중...`);
		const result = await this.prisma.userAssociation.createManyAndReturn(args);
		return result.map((item) => plainToInstance(UserAssociation, item));
	}

	async deleteMany(args: Prisma.UserAssociationDeleteManyArgs): Promise<Prisma.BatchPayload> {
