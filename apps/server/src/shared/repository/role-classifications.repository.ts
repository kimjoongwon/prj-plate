...
export class RoleClassificationsRepository {
	...
	async create(args: Prisma.RoleClassificationCreateArgs): Promise<RoleClassification> {
	...
	async findFirst(args: Prisma.RoleClassificationFindFirstArgs): Promise<RoleClassification> {
		this.logger.debug(`RoleClassification 최초 조회 중...`);
		const result = await this.prisma.roleClassification.findFirst(args);
		return plainToInstance(RoleClassification, result);
	}

	async findUnique(args: Prisma.RoleClassificationFindUniqueArgs): Promise<RoleClassification> {
	...
	async createManyAndReturn(args: Prisma.RoleClassificationCreateManyArgs): Promise<RoleClassification[]> {
		this.logger.debug(`RoleClassification 다중 생성 중...`);
		const result = await this.prisma.roleClassification.createManyAndReturn(args);
		return result.map((item) => plainToInstance(RoleClassification, item));
	}

	async deleteMany(args: Prisma.RoleClassificationDeleteManyArgs): Promise<Prisma.BatchPayload> {
