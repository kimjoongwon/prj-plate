...
export class SpaceAssociationsRepository {
	...
	async create(args: Prisma.SpaceAssociationCreateArgs): Promise<SpaceAssociation> {
	...
	async findFirst(args: Prisma.SpaceAssociationFindFirstArgs): Promise<SpaceAssociation> {
		this.logger.debug(`SpaceAssociation 최초 조회 중...`);
		const result = await this.prisma.spaceAssociation.findFirst(args);
		return plainToInstance(SpaceAssociation, result);
	}

	async findUnique(args: Prisma.SpaceAssociationFindUniqueArgs): Promise<SpaceAssociation> {
	...
	async createManyAndReturn(args: Prisma.SpaceAssociationCreateManyArgs): Promise<SpaceAssociation[]> {
		this.logger.debug(`SpaceAssociation 다중 생성 중...`);
		const result = await this.prisma.spaceAssociation.createManyAndReturn(args);
		return result.map((item) => plainToInstance(SpaceAssociation, item));
	}

	async deleteMany(args: Prisma.SpaceAssociationDeleteManyArgs): Promise<Prisma.BatchPayload> {
