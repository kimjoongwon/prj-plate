...
import { Prisma, UserClassification, UseEntity } from "@shared/schema";
...
export class UserClassificationsRepository {
	...
	async create(args: Prisma.UserClassificationCreateArgs): Promise<UserClassification> {
	...
	async findFirst(args: Prisma.UserClassificationFindFirstArgs): Promise<UserClassification> {
		this.logger.debug(`UserClassification 최초 조회 중...`);
		const result = await this.prisma.userClassification.findFirst(args);
		return plainToInstance(UserClassification, result);
	}

	async findUnique(args: Prisma.UserClassificationFindUniqueArgs): Promise<UserClassification> {
	...
	async createManyAndReturn(args: Prisma.UserClassificationCreateManyArgs): Promise<UserClassification[]> {
		this.logger.debug(`UserClassification 다중 생성 중...`);
		const result = await this.prisma.userClassification.createManyAndReturn(args);
		return result.map((item) => plainToInstance(UserClassification, item));
	}

	async deleteMany(args: Prisma.UserClassificationDeleteManyArgs): Promise<Prisma.BatchPayload> {
