...
describe('GroundPage', () => {
  ...
  describe('현재 섹션/스택 구조 (리팩토링 전)', () => {
    it('sections와 stacks 구조를 가져야 한다', () => {
      const result = getGroundPage('create');

      expect(result.sections).toBeDefined();
      expect(result.sections).toHaveLength(1);
      expect(result.sections[0].stacks).toBeDefined();
      expect(result.sections[0].stacks).toHaveLength(1);
      expect(result.sections[0].stacks[0].type).toBe('VStack');
      expect(result.sections[0].stacks[0].elements).toBeDefined();
    });

    it('ResourceBuilder를 포함해야 한다', () => {
      const result = getGroundPage('create');

      const resourceBuilder = result.sections[0].stacks[0].elements[0];
    ...
    it('Form과 Input 요소들을 포함해야 한다', () => {
      ...
      const resourceBuilder = result.sections[0].stacks[0].elements[0];
      const formElement = resourceBuilder.props.sections[0].stacks[0].elements[0];
